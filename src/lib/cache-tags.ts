...
export function getPageCacheTags(page: 'home' | 'events' | 'info'): string[] {
  ...
  switch (page) {
    case 'home':
      return [...baseTags, CACHE_TAGS.HOME, CACHE_TAGS.ARTIST, CACHE_TAGS.EVENT, CACHE_TAGS.TICKET, CACHE_TAGS.COLLABORATION];
    case 'events':
      return [...baseTags, CACHE_TAGS.EVENTS_PAGE, CACHE_TAGS.EVENT, CACHE_TAGS.ARTIST, CACHE_TAGS.TICKET];
    case 'info':
      return [...baseTags, CACHE_TAGS.INFO_PAGE, CACHE_TAGS.ARTIST, CACHE_TAGS.TICKET, CACHE_TAGS.COLLABORATION];
    default:
      return baseTags;
  }
}
 ...
 */
export function getCacheOptions(tags: string[], revalidateSeconds: number = 3600) {
  return {
    next: {
      tags,
